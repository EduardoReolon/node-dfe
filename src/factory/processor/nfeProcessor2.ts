...
export class NFeProcessor {
    ...
    public async executar(documento: NFeDocumento | NFCeDocumento) {
        ...
        try {
            ...
            let finalResponse = null; // Objeto que conterá o protNFe final
            ...
            if (result.envioNF && result.envioNF.data) {
                ...
                if (data.retEnviNFe) {
                    ...
                    // Lógica Assíncrona (NF-e Modelo 55)
                    if (geral.modelo == '55') {
                        if (retEnviNFe.infRec) {
                    ...
                    // Lógica Síncrona (NFC-e Modelo 65)
                        ...
                        // Na NFC-e o retorno do envio JÁ É o retorno final com protocolo
                ...
                if (finalResponse) {
                    // Sucesso NF-e (Lote Recebido + Lote Processado)
                    if (geral.modelo == '55' && retEnviNFe.cStat == '103' && finalResponse.cStat == '104') {
                        result.confirmada = true;
                        result.success = true;
                    }
                    // Sucesso NFC-e (Lote Processado Direto)
                    if (geral.modelo == '65' && finalResponse.cStat == '104') {
                        result.confirmada = true;
                        result.success = true;
                    }
                }
                ...
                if (arquivos && arquivos.salvar) {
                    ...
                    // Salvar XML de Processamento (Autorizado)
                    if ((result.success == true) && (finalResponse && finalResponse.cStat == '104' && finalResponse.protNFe)) {
                        const filename = `${arquivos.pastaXML}${finalResponse.protNFe.infProt.chNFe}-procNFe.xml`;
                        ...
                        const nfeProc = <schema.TNfeProc>{
                            $: { versao: "4.00", xmlns: "http://www.portalfiscal.inf.br/nfe" }, // Ajustado para 4.00
                            NFe: nfe_enviada.enviNFe.NFe,
                            protNFe: finalResponse.protNFe
                        };
                    ...
                    // Salvar XMLs de Erro / Logs
                    else {
                        const date = new Date().toISOString().replace(/:/g, '-');
                        // Proteção: Se não houver nRec (erro de schema), usa timestamp
                        const idIdentificador = retEnviNFe?.infRec?.nRec || new Date().getTime().toString();
                        
                        const filenameEnvio = `${arquivos.pastaEnvio}${date}-${idIdentificador}-enviNFe.xml`;
                        const filenameRetorno = `${arquivos.pastaRetorno}${date}-${idIdentificador}-retEnviNFe.xml`;
                        
                        if (result.envioNF.xml_enviado) fs.writeFileSync(filenameEnvio, result.envioNF.xml_enviado);
                        if (result.envioNF.xml_recebido) fs.writeFileSync(filenameRetorno, result.envioNF.xml_recebido);
